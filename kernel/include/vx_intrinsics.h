...
// Configure unified cache/shared partition by specifying number of shared memory sets
// sm_sets: number of cache sets allocated for shared memory (remaining sets for L1 cache)
inline void vx_cache_partition(int sm_sets) {
    __asm__ volatile (".insn r %0, 6, 0, x0, %1, x0" :: "i"(RISCV_CUSTOM0), "r"(sm_sets));
}
