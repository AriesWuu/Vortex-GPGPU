  ...
  for (uint32_t row = 0; row < height; ++row) {
    for (uint32_t col = 0; col < width; ++col) {
      ...
      for (uint32_t e = 0; e < width; e += 4) {
        // Pack 4 consecutive int8_t elements from A (row-major)
        int32_t A_pack = *(reinterpret_cast<const int32_t*>(&A[row * width + e]));
        
        // Pack 4 int8_t elements from B (column access)
        // B[k][j] = B[k * width + j] in row-major layout
        int32_t B_pack = ((int32_t)(int8_t)B[(e+0) * width + col] << 0)
                       | ((int32_t)(int8_t)B[(e+1) * width + col] << 8)
                       | ((int32_t)(int8_t)B[(e+2) * width + col] << 16)
                       | ((int32_t)(int8_t)B[(e+3) * width + col] << 24);
...
int main(int argc, char *argv[]) {
  ...
  uint32_t input_buf_size = size_sq * sizeof(INPUT_TYPE);   // A and B buffers (int8)
  uint32_t output_buf_size = size_sq * sizeof(OUTPUT_TYPE); // C buffer (int32)

  std::cout << "input data type: " << "int8" << std::endl;
  ...
  for (uint32_t i = 0; i < size_sq; ++i) {
    h_A[i] = static_cast<INPUT_TYPE>(rand() % 128 - 64);  // Random int8 values -64 to 63
    h_B[i] = static_cast<INPUT_TYPE>(rand() % 128 - 64);  // Random int8 values -64 to 63
  }
