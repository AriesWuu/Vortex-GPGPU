#!/usr/bin/env python3
"""
Plot performance results from simx unified cache sweep tests.
Generates bar charts for: cycles, IPC, L1 hit ratio, L1 access, DRAM access, memory latency.
"""
...
# Configuration
RESULTS_DIR = "/home/kejunwu/vortex/project/simx_results"
OUTPUT_DIR = "/home/kejunwu/vortex/project/simx_plots"

# SM sets configurations (in order)
SM_SETS_LIST = ['separate', '0', '4', '8', '16', '32', '64', '96', '112']
...
def collect_all_results():
    ...
    for app_dir in glob.glob(os.path.join(RESULTS_DIR, "*_w*_t*")):
        ...
        for sm_sets in ['0', '4', '8', '16', '32', '64', '96', '112']:
...
def plot_metric(results, app, metric_key, ylabel, title, filename, 
    ...
    """Plot a single metric as grouped bar chart."""
    
    configs = sorted([c for c in results[app].keys()])
    if not configs:
    ...
    ax.set_title(f'{title} - {app.upper()}')
    ax.set_xticks(x)
    ax.set_xticklabels([SM_SETS_LABELS.get(s, s) for s in SM_SETS_LIST], fontsize=8)
    ax.legend(title='Config (warps_threads)', loc='upper right')
...
def plot_combined_hit_ratio(results, app, filename):
    """Plot combined read and write hit ratios."""
    configs = sorted([c for c in results[app].keys()])
    if not configs:
        return
    ...
    for i, config in enumerate(configs):
        ...
        offset = (i - n_configs/2 + 0.5) * width
        ax1.bar(x + offset, values, width, label=config, color=colors[i])
    
    ax1.set_xlabel('Cache Configuration')
    ax1.set_ylabel('Hit Ratio (%)')
    ax1.set_title(f'L1 Cache Read Hit Ratio - {app.upper()}')
    ax1.set_xticks(x)
    ax1.set_xticklabels([SM_SETS_LABELS.get(s, s) for s in SM_SETS_LIST], fontsize=7)
    ax1.legend(title='Config', loc='lower left')
    ax1.set_ylim(0, 105)
    ...
    for i, config in enumerate(configs):
        ...
        offset = (i - n_configs/2 + 0.5) * width
        ax2.bar(x + offset, values, width, label=config, color=colors[i])
    
    ax2.set_xlabel('Cache Configuration')
    ax2.set_ylabel('Hit Ratio (%)')
    ax2.set_title(f'L1 Cache Write Hit Ratio - {app.upper()}')
    ax2.set_xticks(x)
    ax2.set_xticklabels([SM_SETS_LABELS.get(s, s) for s in SM_SETS_LIST], fontsize=7)
    ax2.legend(title='Config', loc='lower left')
    ax2.set_ylim(0, 105)
...
def plot_l1_access(results, app, filename):
    """Plot L1 cache total accesses (reads + writes)."""
    configs = sorted([c for c in results[app].keys()])
    if not configs:
        return
    ...
    ax.set_title(f'L1 Cache Total Accesses - {app.upper()}')
    ax.set_xticks(x)
    ax.set_xticklabels([SM_SETS_LABELS.get(s, s) for s in SM_SETS_LIST], fontsize=8)
...
def plot_l1_misses(results, app, filename):
    """Plot L1 cache total misses (read misses + write misses)."""
    configs = sorted([c for c in results[app].keys()])
    if not configs:
        return
    ...
    ax.set_title(f'L1 Cache Total Misses - {app.upper()}')
    ax.set_xticks(x)
    ax.set_xticklabels([SM_SETS_LABELS.get(s, s) for s in SM_SETS_LIST], fontsize=8)
    ax.legend(title='Config', loc='upper right')
    ax.grid(axis='y', alpha=0.3)
